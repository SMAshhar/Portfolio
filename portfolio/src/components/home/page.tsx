...
export default function Base() {
    ...
    return (
        <div className='self-start w-full h-full'>
            {/*  TopBar  */}
            <div className="flex ASDGL w-full h-1/6 justify-center rounded-b-full bg-slate-800 bg-opacity-50">
                ...
                <Link href={"/"} onClick={() => setPage("home")} className={`absolute -bottom-1/2 w-32 h-32 rounded-full bg-pink-600 bg-opacity-15 ring-4 ring-pink-600`}>
                    <Image src='/portrait2.jpeg' alt="portrait" fill className="absolute -bottom-1/2 rounded-full bg-cover -z-50" />
                </Link>
            </div>
            {/* Content */}
